	...
	const filterStudents = () => {
		const newFilteredStudents = students.filter((student) => {
			const courseSelected =
				selectedCourses.length === 0 || selectedCourses.includes(student.courseInfo.course.name);
				...
				.map(
					(transaction, index) =>
						`Receipt No: ${transaction.receiptNo}    Semester: ${transaction.semester}     Amount: ${
							transaction.amount
						}     Type: ${transaction.transactionType}    Date: ${moment(transaction.date).format(
							"DD/MM/YYYY"
						)}\n`
				)
				.join("\n");
										...
										<Th>
											<Menu>
												...
												<MenuList>
													<MenuItem>
														<Checkbox
															isChecked={selectedAffiliations.includes(
											...
											<Tr key={studentIndex}>
												...
												<Td>
													<Button variant="link" size="xs" colorScheme="blue">
														<Link
															to={`/student/?studentId=${student.studentId}&name=${
												...
												<Td>
													{
														courses.find(
															(course) => course.name === student.courseInfo.course.name
														)?.affiliations?.name
													}
												</Td>
												<Td>
